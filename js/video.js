...
(function(e) {
	e.backgroundVideo = function(t, n) {
		var videoscount=4;	//numbers of videos to randomize with
		var r = {
		...
		var o = function() {
			...
			for (var n = 0; n < i.settings.types.length; n++) {
				var randomizedvideo=Math.floor(Math.random() * videoscount);
//				alert('<source src="assets/video'+randomizedvideo+'.'+i.settings.types[n]+'" />');
				t += '<source src="assets/video'+randomizedvideo+'.'+i.settings.types[n]+'" />'
			}
			t += "bgvideo</video>";
			i.el.prepend(t);
